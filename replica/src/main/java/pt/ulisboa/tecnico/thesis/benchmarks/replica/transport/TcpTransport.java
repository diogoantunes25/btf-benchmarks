...
public class TcpTransport implements Transport<String> {
    ...
    private void init(Replica me, Integer numChannels) throws IOException, InterruptedException {
        logger.info("Running TCP server at port {}.", me.getPort());
        ...
        for (Replica replica: replicas) {
            ...
            for (int cid=0; cid<numChannels; cid++) {
                ...
                connections.put(key, new Connection(cid, me, replica));
                logger.info("Attempting connection to replica {} on channel {}.", replica.getId(), cid);
            }
        }
        ...
        Thread connThread = new Thread(() -> {
            try {
                while (connectedLatch.getCount() > 0) {
                    logger.info("Missing {} connections.", connectedLatch.getCount());
                    ...
                    logger.info("Connection from replica {} on channel {}.", remoteId, cid);
